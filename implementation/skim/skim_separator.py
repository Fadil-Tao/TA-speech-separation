...
class SkiMSeparator(AbsSeparator):
    ...
    def __init__(self, input_dim: int, causal: bool=True, num_spk: int=2, predict_noise: bool=False, nonlinear: str='relu', layer: int=3, unit: int=512, segment_size: int=20, dropout: float=0.0, mem_type: str='hc', seg_overlap: bool=False):
        super().__init__()
        ...
        self.skim = SkiM(input_size=input_dim, hidden_size=unit, output_size=input_dim * self.num_outputs, dropout=dropout, num_blocks=layer, bidirectional=not causal, norm_type='cLN' if causal else 'gLN', segment_size=segment_size, seg_overlap=seg_overlap, mem_type=mem_type)
    ...
    def forward(self, input: Union[torch.Tensor, ComplexTensor], ilens: torch.Tensor, additional: Optional[Dict]=None) -> Tuple[List[Union[torch.Tensor, ComplexTensor]], torch.Tensor, OrderedDict]:
        ...
        others = OrderedDict(zip(['mask_spk{}'.format(i + 1) for i in range(len(masks))], masks))
        if self.predict_noise:
