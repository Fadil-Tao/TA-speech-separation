...
MODEL_CONFIG = {'encoder': {'channel': 256, 'kernel_size': 16, 'stride': 8}, 'decoder': {'channel': 256, 'kernel_size': 16, 'stride': 8}, 'separator': {'input_dim': 256, 'causal': False, 'num_spk': 3, 'predict_noise': False, 'nonlinear': 'relu', 'layer': 4, 'unit': 256, 'segment_size': 150, 'dropout': 0.2, 'mem_type': 'hc', 'seg_overlap': False}}
...
def resolve_resume_path(resume_from):
    ...
    if len(input_path.parts) == 1:
        ...
        if len(matches) > 1:
            ...
            raise FileNotFoundError(f'Multiple checkpoints matched your filename. Please pass one full path:\n{match_list}')
    searched = '\n'.join([f'- {p}' for p in candidates])
    raise FileNotFoundError(f'Checkpoint not found. Searched these paths:\n{searched}\nHint: use a path like checkpoints/3speaker/skim/checkpoint_epoch_30.pth')
...
def main(resume_from=None, num_epochs=None, dataset_dir=None, checkpoint_dir=None):
    ...
    if resume_from is not None:
        ...
        print(f'✓ Resumed from epoch {start_epoch - 1}. Continuing until epoch {target_num_epochs}.')
    if start_epoch > target_num_epochs:
        print(f'Checkpoint is already at epoch {start_epoch - 1}, which is >= target epoch {target_num_epochs}.')
        return
    ...
    try:
        for epoch in range(start_epoch, target_num_epochs + 1):
            ...
            if epoch % 10 == 0:
                ...
                torch.save({'epoch': epoch, 'model_state_dict': model.state_dict(), 'optimizer_state_dict': optimizer.state_dict(), 'scheduler_state_dict': scheduler.state_dict(), 'scaler_state_dict': scaler.state_dict(), 'train_loss': train_loss, 'val_loss': val_loss, 'best_val_loss': best_val_loss, 'config': MODEL_CONFIG, 'train_losses': train_losses, 'val_losses': val_losses}, checkpoint_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num-epochs', type=int, default=None, help='Total epochs to train up to (default from TRAIN_CONFIG)')
